        ...
        while True:
            try:
                ...
                for fixture in targets:
                    ...
                    # ── FILTRE MOYENNE DE BUTS VS SCORE ACTUEL ───────────────
                    total_now = gh + ga
                    if total_now >= 1:
                        avg_h = hist_home.get("avg5", 0) if hist_home else 0
                        avg_a = hist_away.get("avg5", 0) if hist_away else 0
                        avg_combined = round((avg_h + avg_a) / 2, 1) if (avg_h or avg_a) else 0
                        if total_now >= 2 and avg_combined < 3.0:
                            log.info(f"SKIP {home_name} vs {away_name} : {total_now} buts mais avg={avg_combined} < 3.0")
                            continue
                        if total_now == 1 and avg_combined < 2.0:
                            log.info(f"SKIP {home_name} vs {away_name} : {total_now} but mais avg={avg_combined} < 2.0")
                            continue
                    # ─────────────────────────────────────────────────────────
...
async def main():
    ...
    await bot.send_message(
        ...
        text=(
            ...
            "• Skip si score > avg historique des équipes\n"
